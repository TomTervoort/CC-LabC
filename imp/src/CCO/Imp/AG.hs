...
--   be used to 'feed' an argument to a couple of functions combined with >>>.
(<<$) :: a -> (a -> b) -> b
(<<$) x f = f x
...
get x (local : global) = case lookup x (vars local) of
  Nothing         -> {-- trap 1 . getCached x (local:global) --} ldl (- 2) . getGlobal x global
